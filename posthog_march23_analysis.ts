...
function processEvents(views: any[], starts: any[], completes: any[], bookings: any[], rawEmails: any[]) {
    ...
    // Ordenar de completados/agendados a solo vistos
    userFunnel.sort((a, b) => {
...
async function run() {
    console.log("=========================================");
    console.log("Extrayendo datos de Posthog (2 y 3 de Marzo)...");

    const afterDate = "2026-03-02T00:00:00Z";
    const beforeDate = "2026-03-03T23:59:59Z";
    ...
    let outputString = [
        "=== REPORTE POSTHOG (2 Mar - 3 Mar 2026) ===",
        ...
        "\n📈 EVENTOS GLOBALES DE ESOS DÍAS",
        `  Total eventos procesados por la API: ${totalEvents}`,
        JSON.stringify(eventCounts, null, 2),
    ].join('\n');

    fs.writeFileSync('posthog_march23_output.txt', outputString, 'utf-8');
    console.log("¡Reporte escrito a posthog_march23_output.txt!");
}
